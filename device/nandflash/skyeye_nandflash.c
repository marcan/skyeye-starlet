...
static nandflash_module_option nandflash_module_data[]={
	...
	{"HY27UF084G2M", 2048, 64, 64, 4096, {0xAD,0xDC,0x80,0x95}, nandflash_lb_setup, nandflash_lb_uninstall},
};

extern void nandflash_s3c2410_init(struct device_module_set *mod_set);
extern void nandflash_starlet_init(struct device_module_set *mod_set);
...
nandflash_init (struct device_module_set *mod_set)
{
  //	nandflash_s3c2410_init (mod_set);
	nandflash_starlet_init (mod_set);
}
...
nandflash_setup (struct device_desc *dev, void *option)
{
	...
	memcpy (nandflash_dev->dump, nandflash_opt->dump, MAX_STR_NAME);
	nandflash_dev->dumptype =  nandflash_opt->dumptype;
