	...
	MIPS_State * mstate = (MIPS_State *)state;
	 /* uart write word */
        if(addr >= 0x11100000 && addr <= (0x11400000 + 0x10)){
                return au1100_uart_read_word(addr & 0x600000, state, addr & 0xfff);
        }
...
au1100_io_write_word(void * state, UInt32 addr, UInt32 data)
{
	...
	/* uart write word */
	if(addr >= 0x11100000 && addr <= (0x11400000 + 0x10)){
		au1100_uart_write_word(addr & 0x600000, state, addr & 0xfff, data);
		return;
	}
	switch (addr) {
		case 0x11900004:
...
static void
au1100_clear_int(UInt32 irq)
{

}

static void
au1100_set_int(UInt32 irq)
{

}

static void
au1100_sti(UInt32 dreg)
{

}

static void
au1100_cli(UInt32 dreg)
{
...
au1100_mach_init (void * state, machine_config_t * this_mach)
{
	...
	this_mach->mach_io_read_byte = au1100_io_read_byte;
        ...
        this_mach->mach_set_intr = au1100_set_int;
