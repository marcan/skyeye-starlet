...
mips_step_once()
{
	...
	//fprintf(skyeye_logfd, "KSDBG:instr=0x%x,pa=0x%x, va=0x%x, sp=0x%x, ra=0x%x,ra_mem=0x%x\n", instr, pa, va, mstate->gpr[29], mstate->gpr[31],mips_mem.rom[0][(0x3c5ed8 >> 2)]);
	...
	if((mstate->cp0[SR] & (1 << SR_IEC)) && (mstate-> cp0[SR] & 1 << SR_IM7)){
		if(!(mstate->cp0[Cause] & 1 << Cause_IP7))
		{
...
static machine_config_t mips_machines[] = {
...
static int mips_ICE_write_byte(WORD addr, uint8_t data){
	mips_mem_write(addr, (UInt32 *)&data, 1);
	return 0;
}
