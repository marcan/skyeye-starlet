...
/*		if ((va & (WORD_SIZE - 1)) && MMU_Aligned) {
		...
		if (fault) {
			d_msg ("I translate fault VA=%08x\n",va);
			//skyeye_exit(1);
			return fault;
		}
		    ...
		    ARMword datatype)
{
	...
	if (((va & 3) && (datatype == ARM_WORD_TYPE) && MMU_Aligned) ||
		...
		d_msg ("D Read align fault @%08x (%d)\n", va, datatype);
		//skyeye_exit(1);
		return ALIGNMENT_FAULT;
	}			// else
	...
	fault = translate (state, va, ARM926EJS_MAIN_TLB (), &tlb);
	if (fault) {
		d_msg ("D Read translate fault VA=%08x\n", va);
		return fault;
	}
	/*check access permission */
	fault = check_access (state, va, tlb, 1);
	if (fault) {
		d_msg ("D Read access fault VA=%08x\n", va);
		return fault;
	}
...
*/
	...
	fault = translate (state, va, ARM926EJS_MAIN_TLB (), &tlb);
	if (fault) {
		d_msg ("D Write translate fault VA=%08x\n", va);
		return fault;
	}
	/*tlb check access */
	fault = check_access (state, va, tlb, 0);
	if (fault) {
		d_msg ("D Write access fault VA=%08x\n", va);
		d_msg ("check_access\n");
		return fault;
	}
	/*search main cache */
...
ARMword arm926ejs_mmu_cache_ops (ARMul_State * state, ARMword instr, ARMword value);
...
arm926ejs_mmu_mrc (ARMul_State * state, ARMword instr, ARMword * value)
{
	...
	case MMU_CACHE_OPS:
		/* TODO */
		data = arm926ejs_mmu_cache_ops(state, instr, 0);
		break;
...
arm926ejs_mmu_cache_ops (ARMul_State * state, ARMword instr, ARMword value)
{
	...
	printf("CACHE Unknown OPC_2 = %x CRm = %x\n", OPC_2, CRm);
	return 0;
}
...
arm926ejs_mmu_mcr (ARMul_State * state, ARMword instr, ARMword value)
{
	...
	int OPC_2 = BITS (5, 7) & 0x7;
//	printf("mmu_mcr - reg %d d %08x\n", creg,value);
	switch (creg) {
	case MMU_CONTROL:
		printf("mmu_mcr wrote CONTROL %08x\n", value);
		state->mmu.control = (value | 0x50078) & 0x0005F3FF;
		break;
	case MMU_TRANSLATION_TABLE_BASE:
		printf("mmu_mcr wrote TTB %08x\n", value);
		state->mmu.translation_table_base = value & 0xFFFFC000;
		break;
	case MMU_DOMAIN_ACCESS_CONTROL:
		//printf("mmu_mcr wrote DACR %08x\n", value);
