...
mem_reset (ARMul_State * state)
{
	...
	for (i = 0; i < num; i++) {
		...
		if (mb[bank].type != MEMTYPE_IO && mb[bank].type != MEMTYPE_ALIAS) {
...
ARMword alias_read_byte (ARMul_State * state, ARMword addr)
{
	return mem_read_byte(state, addr - global_mbp->addr + global_mbp->dest);
}

ARMword alias_read_halfword (ARMul_State * state, ARMword addr)
{
	return mem_read_halfword(state, addr - global_mbp->addr + global_mbp->dest);
}

ARMword alias_read_word (ARMul_State * state, ARMword addr)
{
	return mem_read_word(state, addr - global_mbp->addr + global_mbp->dest);
}

void alias_write_byte (ARMul_State * state, ARMword addr, ARMword data)
{
	return mem_write_byte(state, addr - global_mbp->addr + global_mbp->dest, data);
}
void alias_write_halfword (ARMul_State * state, ARMword addr, ARMword data)
{
	return mem_write_halfword(state, addr - global_mbp->addr + global_mbp->dest, data);
}
void alias_write_word (ARMul_State * state, ARMword addr, ARMword data)
{
	return mem_write_word(state, addr - global_mbp->addr + global_mbp->dest, data);
}
