		...
		if (temp) {
			...
			case 0x12:	/* TEQ reg and MSR reg to CPSR (ARM6).  */
				...
				if (BITS (4, 27) == 0x12FFF1) {
					/* BX */
//					if(RHSReg==0) print_ret(state->Reg[RHSReg]);
					WriteR15Branch (state,
							state->Reg[RHSReg]);
					break;
				}
			...
			case 0xaf:
				...
				FLUSHPIPE;
				//if(pc != 0xffff1dc0) printf("%08x: B %08x\n", pc, state->Reg[15]);
			...
			case 0xff:
				...
				if(state->undf_trap) {
					int res;
					res = state->undf_trap(state, instr);
					if(!res) ARMul_Abort (state, ARMul_SWIV);
				} else {
