...
void print_call(ARMword dest);

void print_ret(ARMword dest);
     ...
     ARMword *ainstr;
{
	...
	*ainstr = 0xDEADC0DE;
	...
	case 31:		/* BL instruction 2 */
		...
		{
			ARMword tmp = (tinstr & 0xF800) >> 11;
			if(!state->is_v5 && tmp == 29) {
				valid = t_undefined;
				break;
			}
			ARMword tmp2 = (pc + 2);
			ARMword dest = state->Reg[14] + ((tinstr & 0x07FF) << 1);
			state->Reg[14] = (tmp2 | 1);
			if(tmp == 29) { // BLX
				CLEART;
				state->Reg[15] = dest & 0xfffffffc;
