...
int ppc_effective_to_physical(uint32 addr, int flags, uint32 *result){
	...
	if(ppc_exception(DATA_TLB, 0, addr))
		return PPC_MMU_EXC;
...
{
	//fprintf(stderr,"tlbivax is called, but not implemented\n");
	int i,j;
	for(i = 0; i < L2_TLB0_SIZE; i++)
		if(!l2_tlb0_4k[i].iprot)
			l2_tlb0_4k[i].v = 0;
	for(j = 0; j < L2_TLB1_SIZE; j++)
		if(!l2_tlb1_vsp[i].iprot)
			l2_tlb1_vsp[i].v = 0;
}
/*
...
void ppc_opc_tlbwe()
{
	...
	if(TLBSEL(e500_mmu.mas[0]) == 0x0){
		//offset = ((ESEL(e500_mmu.mas[0]) & 0x1) << 7) | (EPN(e500_mmu.mas[2]) & 0x7f);
	...
	//printf("In %s, rpn=0x%x, epn=0x%x, offset=0x%x, mas0=0x%x, mas2=0x%x\n", __FUNCTION__, entry->rpn, entry->epn, offset, e500_mmu.mas[0], e500_mmu.mas[2]);
}
...
void ppc_opc_tlbsx(){
	...
	while(i++ < (L2_TLB0_SIZE + L2_TLB1_SIZE)){
        	if(i > (L2_TLB0_SIZE - 1)){
                ...
                if(entry->ts & 0x0)
                        continue;
		if(entry->tid != 0 && entry->tid != ((e500_mmu.mas[6] & 0xFF00) >> 8)){
                                continue;
                }
		if(i > (L2_TLB0_SIZE - 1)){
                        int k,s = 1;
		...
		if((addr & mask) == (entry->epn << 12)){
			if(i > (L2_TLB0_SIZE - 1)){
				...
				e500_mmu.mas[0] &= ~(0x1 << 28);
				e500_mmu.mas[0] = (e500_mmu.mas[0] & 0xFFF0FFFF) | (i << 16);
			}
