...
inline static void ppc_opc_group_2()
{
	uint32 ext = PPC_OPC_EXT(gCPU.current_opc);
	/*
	if(gCPU.pc >= 0xfff80100 && gCPU.pc < 0xfffff000)
                        printf("DBG:before exec pc=0x%x,opc=0x%x,ext=0x%x\n", gCPU.pc, gCPU.current_opc, ext);
	*/
...
//ppc_opc_function * ppc_opc_table_groupv;
