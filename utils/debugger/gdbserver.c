		 ...
		 char *to)
{
	...
	*mem_addr_ptr = *len_ptr = 0;
	...
	convert_ascii_to_int (&from[i++], (unsigned char *)to, *len_ptr);
}
...
decode_Frame_packet(char *buffer, char* response)
{
	...
	switch (buffer[0])
	{
		...
		case 't': //tdp
			...
			if (select_frame_buffer_by_tpn (tracepoint_number,(int *)&frame_number))
			{
		...
		default: //a frame number or an error
			{
				...
				{
					if (select_frame_buffer_by_fn ((int *)&tracepoint_number,frame_number))
					{
						sprintf (response,"F%xT%x",frame_number,tracepoint_number);
					}
					else
...
void swabx(unsigned char *mem, int size) {
	...
	while (1) {
		...
		while (getpkt (own_buf) > 0) {
			...
			case 'g':
				...
				swabx(registers, current_reg_type->register_bytes);
			...
			case 'm':
				...
				if ((get_trace_status() == TRACE_FOCUSING) && (is_in_ro_region(mem_addr,len) == 0))
					size = trace_read (mem_addr, (unsigned char *)mem_buf, len);
				else
					size = sim_read (mem_addr, (unsigned char *)mem_buf, len);
				if(size!=-1)
					convert_int_to_ascii ((unsigned char *)mem_buf, own_buf, len);
				else 
					write_enn (own_buf);
				break;
			case 'M':
				...
				if (sim_write (mem_addr, (unsigned char *)mem_buf, len) == len)
					write_ok (own_buf);
				else
					write_enn (own_buf);
				break;
