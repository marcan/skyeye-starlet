...
#if !(defined(NO_BFD) || defined(__MINGW32__))
...
{
	int i,fault;
	addr = (addr & load_addr_mask)|load_addr;
	for (i = 0; i < size; i++) {
...
tea_load_exec(const char *file, int only_check_big_endian)
{
	...
	int tmp_fd;
	int r;
	struct Elf32_Header *elfFile=NULL;
	struct IOSBinary *iosFile=NULL;
	struct stat stat;
	...
	iosFile = (void*)elfFile;

	big_endian = 1;
	if (e2h32(iosFile->entry) <= 0x20 && e2h32(iosFile->reserved) == 0) {
		if (only_check_big_endian) goto out;
		uint32_t base = 0x10100000;
		size_t filesz = stat.st_size;
		tea_write(base, (unsigned char *)iosFile, filesz);
		if (skyeye_config.start_address == 0) {
			skyeye_config.start_address = e2h32(iosFile->entry) + base;
		}
	} else {
		...
		printf("ELF loading: bigendian %d\n",big_endian);
		...
		for(i=0; i < e2h16(elfFile->e_phnum); i++) {
			...
			dest = e2h32(segments[i].p_vaddr);

			src = ((char*) elfFile) + e2h32(segments[i].p_offset);
			if(e2h32(segments[i].p_type) == 1 ) {
				printf("ELF section: 0x%08x 0x%08x 0x%08x 0x%08x\n", (uint32_t)filesz, (uint32_t)memsz, (uint32_t)dest, (uint32_t)(src-(char *)elfFile));
				tea_write(dest, (unsigned char *)src, filesz);
				dest += filesz;
...
//AJ2D--------------------------------------------------------------------------
	fprintf (stderr, /* 2007-03-29 by Anthony Lee : for specify big endian when non ELF */
...
//AJ2D--------------------------------------------------------------------------
		...
		case 'l':
		{
			...
			printf("Your ELf file is loaded to another address,not its entry\n");
			load_addr = strtoul(str1, NULL, 16);
			load_addr_mask = strtoul(str2, NULL, 16);
		}
			break;
		case 'b':
			big_endian = 1;
...
//teawater add for load elf 2005.07.31------------------------------------------
	...
	//AJ2D--------------------------------------------------------------------------
	if (skyeye_config.start_address != 0){
		unsigned long addr = (skyeye_config.start_address & load_addr_mask)|load_addr;
