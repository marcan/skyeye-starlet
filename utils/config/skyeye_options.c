...
int setup_device (char *dev_name, struct common_config *conf,void *option, machine_config_t * mc);
		 ...
		 const char *params[])
{
	...
	for (i = 0; i < num_params; i++) {
		...
		if (!strcmp("dump", name)) {
			memcpy(flash_opt.dump, value, strlen(value) + 1);
		}
		if (!strcmp("dumptype", name)) {
			flash_opt.dumptype = atoi(value);
		}
	}
...
int do_regfile_option(){return 0;}
